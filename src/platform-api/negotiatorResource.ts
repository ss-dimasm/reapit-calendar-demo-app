...
export const getAppointmentDateByNegotiatorId = async (
  ...
  try {
    ...
    console.error(
      'Error fetching Appointment Date by Negotiator Id Result',
      err
    )
  }
}
...
/**
 * Get Paged Negotiators Data by Person Name
 */
export const getNegotiatorDataByNegotiatorName = async (
  session: ReapitConnectSession,
  searchParams: string | undefined
): Promise<NegotiatorModelPagedResult | any> => {
  const req = axios.get(
    `${window.reapit.config.platformApiUrl}${URLS.NEGOTIATOR.PAGED}?name=${searchParams}`,
    {
      headers: {
        ...BASE_HEADERS,
        Authorization: `Bearer ${session.accessToken}`,
