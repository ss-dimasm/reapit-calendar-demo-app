...
const Negotiator: FC<NegotiatorProps> = (): ReactElement => {
  ...
  const [negotiatorsListdata, setNegotiatorsListData] = useState<
    NegotiatorModelPagedResult | undefined
  >(undefined)
  const [selectedNegotiatorId, setSelectedNegotiatorId] = useState<
    string | undefined
  >(undefined)
  const [singleNegotiatorData, setSingleNegotiatorData] = useState<
    NegotiatorModel | undefined
  >(undefined)

  useEffect(() => {
    if (!connectSession) return
    const getNegotiatorByName = async (): Promise<
      NegotiatorModelPagedResult | undefined
    > => {
      if (!connectSession || !searchQuery || selectedNegotiatorId) return
      console.log('negotiator name called')
      const serviceResponse = await getNegotiatorDataByNegotiatorName(
        connectSession,
        searchQuery
      )
      if (serviceResponse) {
    ...
    const getNegotiatorById = async (): Promise<
      NegotiatorModel | undefined
    > => {
  ...
  if (isPageLoading)
    return <Loading text="Please wait..." isCenter isFullScreen />
