...
const Calendar: FC = (): ReactElement => {
  ...
  const [propertiesData, setPropertiesData] =
    useState<PropertyModelPagedResultVamp>(undefined)

  useEffect(() => {
    const fetchPagedProperties =
      async (): Promise<PropertyModelPagedResultVamp> => {
        ...
        const serviceResponse = await getListingsOfProperties(
          connectSession,
          propertyData
        )
  ...
  if (propertiesData === undefined) {
    return (
      <>
        <SearchBar
          searchNewParams={searchNewList}
          resetPropertiesData={resetPropertiesData}
        />
