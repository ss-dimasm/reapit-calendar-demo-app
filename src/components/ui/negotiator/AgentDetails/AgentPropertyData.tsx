...
import { PropertyImageModelPagedResult, PropertyModel } from '@reapit/foundations-ts-definitions';
...
const AgentPropertyData: FC<AgentPropertyDataType> = (props): ReactElement => {
	...
	const [propertyData, setPropertyData] = useState<PropertyModel | undefined>(undefined);
	const [propertyImagesData, setPropertyImageData] = useState<PropertyImageModelPagedResult | undefined>(undefined);
	const [isPropertyLoading, setIsPropertyLoading] = useState<boolean>(true);

	useEffect(() => {
		if (!connectSession) return;

		const getPropertyDetailsDataByPropertyId = async (): Promise<PropertyModel | void> => {
		...
		const getPropertyImagesDetailsDataByPropertyId = async (): Promise<PropertyImageModelPagedResult | void> => {
			if (!propertyId) return;
	...
	if (isPropertyLoading) return <Loading text='Getting Property Information...' isCenter />;
	...
	return (
		<FlexContainer isFlexJustifyBetween>
			...
			<div style={{ width: '50%' }}>
				<InputGroup icon='geoLocationSolidSystem' label='Address' defaultValue={propertyAddress} />
				<InputGroup icon='calendarSystem' label='From' type='datetime-local' disabled defaultValue={dateStart} />
				<InputGroup icon='calendarSystem' label='Until' type='datetime-local' disabled defaultValue={dateEnd} />
