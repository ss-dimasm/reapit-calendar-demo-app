...
const AgentDetails: FC<AgentDetailsType> = (props): ReactElement => {
	...
	const [appointmentsData, setAppointmentsData] = useState<AppointmentModelPagedResult | undefined>(undefined);

	useEffect(() => {
		if (!connectSession) return;
		const getAppointmentsDataBasedNegotiatorId = async (): Promise<AppointmentModelPagedResult | void> => {
			const serviceResponse = await getAppointmentDateByNegotiatorId(connectSession, singleNegotiatorId);
	...
	return (
		<>
			...
			<FlexContainer isFlexJustifyBetween>
				...
				<AgentCalendar negotiatorId={singleNegotiatorId} appointmentData={appointmentsData} />
			</FlexContainer>
		</>
	);
};
