...
const AgentResults: FC<AgentResultsType> = (props): ReactElement => {
  const { searchQuery, agentsData, changePageLocation, viewNegotiatorById } =
    props
    ...
    <div>
      <FlexContainer className="el-mb8">
        <Button
          chevronLeft
          intent="primary"
          onClick={() => changePageLocation('search')}
        >
          Back
        </Button>
      ...
      <FlexContainer isFlexWrap isFlexJustifyCenter>
        {agentsData?._embedded?.map(
          (agentData: NegotiatorModel): ReactElement => {
            return (
              <CardWrap
                key={agentData.id}
                className="el-m5 el-w2"
                onClick={() => viewNegotiatorById(agentData.id)}
              >
