...
const SubTableAppointment: FC<SubTableAppointmentProps> = (
  props
): ReactElement => {
  ...
  const [negotiatorId, setNegotiatorId] = useState<string | undefined>(
    undefined
  )
  const [negotiatorData, setNegotiatorData] =
    useState<NegotiatorDataType>(undefined)
  const [propertyData, setPropertyData] = useState<PropertyDataType>(undefined)
  const [propertyImageData, setPropertyImageData] =
    useState<PropertyImageDataType>(undefined)
  const [appointmentProperty, setAppointmentProperty] =
    useState<AppointmentModelPagedResultVamp>(undefined)
  ...
  const [reservedAppointmentDate, setReservedAppointmentDate] =
    useState<AppointmentDateProps>(undefined)
  const [finalReservedAppointment, setFinalReservedAppointment] = useState<
    CreateAppointmentModelType | undefined
  >(undefined)
  ...
  useEffect(() => {
    ...
    const fetchAppointmentDataByNegotiator =
      async (): Promise<AppointmentModelPagedResultVamp> => {
    ...
    const fetchNegotiatorDataByNegotiatorId =
      async (): Promise<NegotiatorDataType> => {
        if (!connectSession) return

        const serviceResponse = await getNegotiatorDataByNegotiatorId(
          connectSession,
          negotiatorId
        )
    ...
    const fetchPropertyDataByPropertyId =
      async (): Promise<PropertyDataType> => {
        if (!connectSession) return
    ...
    const fetchPropertyDataImagesByPropertyId =
      async (): Promise<PropertyImageDataType> => {
        if (!connectSession) return
  ...
  const changeUserPurpose = (data: string | undefined): void =>
    setUserPurpose(data)
  ...
  const updateReservedAppointmentDate = (
    data: CreateAppointmentModelType
  ): void => setFinalReservedAppointment(data)
            ...
            <CardHeadingWrap className="el-mt6">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: description ?? 'Description unavailable',
                }}
              />
            </CardHeadingWrap>
            <Button
              className="el-ml6"
              intent="secondary"
              onClick={() => openModalProperty(negoId)}
            >
              Book Now
            </Button>
          </FlexContainer>
