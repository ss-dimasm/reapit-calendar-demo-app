...
const ModalSummary: FC<ModalSummaryType> = (props): ReactElement => {
  ...
  useEffect(() => {
    ...
    const postDataToServer = async (): Promise<
      CreateAppointmentModelType | undefined
    > => {
      if (!connectSession) return
      const serviceResponse = await postNewAppointment(
        connectSession,
        finalReservedAppointment
      )
  ...
  const appointmentDateStart = convertTimeToAssignedInput(
    appointmentDateData?.start
  )
  const appointmentDateEnd = convertTimeToAssignedInput(
    appointmentDateData?.end
  )
  ...
  const openGoogleMap = (coordinate: string): Window | null =>
    window.open(coordinate)
  const sendDataToServer = (): void => {
    setIsFetching(true)
    const data = retrieveAppointmentDataPostModel(
      appointmentDateData,
      propertyData,
      negotiatorData,
      userInfoData
  ...
  return (
    <FlexContainer isFlexJustifyBetween>
      ...
      >
        ...
        <CardSubHeadingAdditional className="el-mx-auto">
          {negotiator.name}
        </CardSubHeadingAdditional>
        <CardImageWrap
          className="el-mx-auto"
          style={{ width: '90%', height: '250px', marginRight: '0.75rem' }}
        >
          <img
            alt={negotiator.name}
          ...
          <CardWrap className="el-mb3">
            ...
            <div className="el-my6">
              ...
              <div className="el-flex el-flex-justify-end el-mt6">
                <Button
                  intent="primary"
                  onClick={() => openGoogleMap(propertyCoordinate)}
                >
                  Open in Google Maps
                </Button>
              </div>
