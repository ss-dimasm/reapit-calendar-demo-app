...
import { CompanyModelPagedResult, CompanyModel, StaffModelPagedResult } from '@reapit/foundations-ts-definitions';
...
const ListCompaniesData: FC<EmbeddedCompanyModelPagedResult> = (
  ...
  const [companyIdData, setCompanyIdData] = useState<CompanyIdDataType>(undefined);
  const [companyIdDataStaff, setCompanyIdDataStaff] = useState<CompanyIdStaffType>(undefined);
  ...
  useEffect(() => {
    ...
    const fetchSingleCompanyData = async (): Promise<CompaniesModelPagedVamp> => {
      if (!connectSession) return;
      const serviceResponse = await configurationCompaniesApiServices(connectSession, undefined, companyId);
    ...
    const fetchSingleCompanyEmployeeData = async (): Promise<CompanyIdStaffType> => {
      if (!connectSession) return;
      const serviceResponse = await configurationCompaniesEmployeeApiServices(connectSession, companyId);
  ...
  return (
    <>
      ...
      <Modal isOpen={modalOpen} onModalClose={closeModalEv} title={`Company ID: ${companyId}`}>
