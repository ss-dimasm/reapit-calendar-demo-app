...
const ModalSingleCompanyData: FC<ModalSingleCompanyDataProp> = (
  props: ModalSingleCompanyDataProp
): ReactElement => {
  const { data } = props
  ...
  return (
    <>
      <Subtitle hasBoldText>
        Information {data.name} - {data.branch} (
        <StatusIndicator intent={status} />)
      </Subtitle>
      <div>
